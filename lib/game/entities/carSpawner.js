...
EntityCarSpawner = ig.Entity.extend({
    ...
    update: function() {
        ...
        // Unit test number of cars spawned and still alive
        if(ig.system.runUnitTests)
            this.UTESTcarsAliveValue();
    },

    // -- UNII TESTS -- //

    UTESTcarsAliveValue: function() {
        console.assert(
            this.carsAlive >= 0 && this.carsAlive <= this.maxCars, 
            'Car Spawner has an invalid number of cars spawned and still alive (' + this.carsAlive + ', max cars: ' + this.maxCars + ')'
        );
    },
})
});